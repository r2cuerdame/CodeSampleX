...
Diese Zeile ist keine Illustration — sie ist [die Live-Seite](https://codesamplex.dev/npm/axios/1.12.2/axios.post). `PASS` auf Linux ist ein Contract, der in einem gepinnten Container ausgeführt wurde; die Windows-Zelle ist ehrlich darüber, dass sie schwächere Evidenz trägt (`○` observed mit dem Unsicherheitsmarker `?` — Build-Beobachtungen, kein Contract-Lauf), und über ihre gemessenen Fehlschläge (`!`). Unbekanntes bleibt `—`; nichts wird aus dem Ökosystem des Pakets oder seiner Doku hergeleitet.
