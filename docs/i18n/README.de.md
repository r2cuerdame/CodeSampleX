...
**Eine Zelle trägt eine Quote und genau eine Markierung, nie ein Urteil.** Der Prozentsatz und die Zahl daneben sind Beobachtungen: 82 % von 1.209 aufgezeichneten Beobachtungen kamen durch. Eine Beobachtung ist eine Stufe, die ein Build erreicht hat — kompilieren, typprüfen, testen —, ein einzelner Build hinterlässt also mehrere, und die Zahl zählt weder Builds noch Maschinen noch Personen. Das Zeichen `▤` ist ein Dokument, und ein Dokument sagt genau eines: An dieser Koordinate gibt es ein Beispiel. Seine FARBE sagt, wie unser eigener Lauf hier ausging — grau hier hat es niemand ausgeführt, grün er lief sauber durch, rot er schlug fehl, in der Mitte geteilt beides ist verzeichnet. Ein README kann keine Farbe drucken; lies jedes `▤` oben als ein grünes. Eine Zelle ohne Beispiel trägt gar kein Dokument, denn Code vorzutäuschen, den man nicht öffnen kann, ist schlimmer als eine leere Zelle. Ein grünes `▤ —` heißt: unser Kontrakt lief hier und kam sauber zurück, und an dieser Koordinate wurde noch kein Build gemeldet. `—` bleibt unbekannt.
...
Code: Apache-2.0. Veröffentlichte Samples stehen standardmäßig unter **MIT-0**. Evidence und aggregierte Kompatibilitätsdaten: **CDLA-Permissive-2.0** (siehe [DATA_TERMS.md](../../DATA_TERMS.md)).
