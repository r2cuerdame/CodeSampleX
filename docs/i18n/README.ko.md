...
이 격자는 그린 그림이 아니라 [실제 라이브 페이지](https://codesamplex.dev/golang/github.com%2Fjackc%2Fpgx%2Fv5)입니다.

**셀은 비율과 표시를 담을 뿐 판정을 내리지 않습니다.** 백분율과 그 옆의 숫자는 실제 기계들이 한 일입니다 — 기록된 1,156회 실행 중 85%가 통과했습니다. 체크는 우리 샘플이 거기서 돌다는 뜻입니다 — 한 번이든 백 번이든 사실은 하나고, 우리 실행이 실패했다면 체크가 아예 붙지 않습니다. `PASS`는 없습니다. "PASS"는 *여기서는 된다*는 일반적 주장으로 읽히는데, 실제로 측정한 건 *네 번 돌려 네 번 통과*이기 때문입니다.

둘을 갈라둔 건 의도적입니다. 우리 실행은 고정된 컨테이너 하나의 반복이고, 보고된 천 번의 실행은 천 개의 서로 다른 상황입니다. 더하면 우리 세 번이 같은 종류의 증거인 척하게 됩니다. `✓ —` 셀은 정확히 이걸 말합니다 — 동작하는 코드는 있고, 쓰는 건 아직 아무도 본 적이 없다.
