...
CodeSampleX는 개발자 라이브러리·런타임·툴체인을 위한 **열린 호환성 테스트 네트워크**입니다. 문서를 요약하지도 않고, 일화를 수집하지도 않습니다. 가정하지 않고 기록된 환경에서 **진짜 빌드가 실제로 무엇을 했는지**를 기록한 뒤 — 무엇이 동작했고, 어디서 깨졌으며, 누구의 기계에서 그랬는지를 보여줍니다.

자산은 두 가지이고, 세 번째는 보너스입니다:

- **증거(evidence)** — 실제 개발자 기계에서 돌아간 진짜 빌드. 환경, 도달한 단계, 실패했다면 공개 에러 코드가 함께 기록됩니다. csx가 깔린 곳이면 어디서든 자동으로 들어오기 때문에, 어떤 컨테이너도 갈 수 없는 곳까지 닿습니다 — macOS는 컨테이너 런타임 자체가 없고, npm은 Windows 이미지를 내지 않으며, musl·ARM·사내 베이스 이미지·사람들이 실제로 설치한 런타임 버전의 긴 꼬리는 끝이 없습니다. **지도를 채울 수 있는 것은 이것뿐입니다.**
...
그래서 미스가 빈손이 아닙니다. 당신의 경우에 대해 증명된 것이 없으면 등급은 `NO_SAFE_MATCH`로 남되, **기록된 실행 결과가 기록된 그대로 함께 돌아옵니다.**

- 호환성 지도: **https://codesamplex.dev**
- 이 네트워크가 답하는 질문: *거기서 돌아가나요?* — 이 API가, 이 버전에서, 이 OS 위, 이 런타임에서.
- 이 네트워크가 주는 답: *이런 일이 있었고, 누가 돌렸는지는 이렇습니다.*
...
저 행은 예시로 그린 그림이 아니라 [실제 라이브 페이지](https://codesamplex.dev/npm/axios/1.12.2/axios.post)입니다.

**셀은 비율을 말하고 근거를 밝힐 뿐, 판정을 내리지 않습니다.** `■ verified`는 우리가 고정된 컨테이너에서 직접 실행한 계약이고, `○ observed`는 실제 기계들이 실행하고 보고한 것입니다. 숫자가 곧 측정값 — 실행 대비 통과 — 이라서, 외로운 `1/1`은 증거가 얼마나 얇은지를 그대로 말합니다. 예전에는 그게 백 번 일치한 실행과 똑같은 표시 뒤에 숨어 있었습니다. `PASS`는 없앴습니다. "PASS"는 *여기서는 된다*는 일반적 주장으로 읽히는데, 실제로 측정한 것은 *네 번 돌려서 네 번 통과*이기 때문입니다.

근거는 절대 흐려지지 않습니다. 관측 수는 검증 수를 압도합니다 — 네트워크에는 수천 건의 보고된 실행이 있고 영수증은 그보다 훨씬 적습니다 — 그래서 둘 다 맨 비율로 두면 익명 셀이 증명된 셀보다 권위 있어 보입니다. 글리프는 우리가 돌린 것에는 채워진 모양, 보고받은 것에는 빈 모양이라 색 없이도 구별됩니다. 색은 비율이 어떻게 나왔는지만 나릅니다 — 실패는 드물고 정보량이 큰 사건이라 눈에 걸려야 하니까요. `!`는 측정된 이상, `?`는 약하거나 오래된 증거, `—`는 알 수 없음으로 남습니다. 패키지가 속한 생태계나 문서로부터 아무것도 추론하지 않습니다.
