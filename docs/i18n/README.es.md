...
**Una celda lleva una tasa y una sola marca, nunca un veredicto.** El porcentaje y el número contiguo son observaciones: el 82% de 1.209 observaciones registradas pasó. Una observación es una etapa que alcanzó un build —compilar, comprobar tipos, probar—, así que un solo build deja varias, y el número no cuenta builds, ni máquinas, ni personas. La marca `▤` es un documento, y un documento dice una sola cosa: en esta coordenada hay una muestra. Su COLOR dice cómo fue nuestra propia ejecución aquí: gris nadie la ha ejecutado aquí, verde pasó, rojo falló, partido por la mitad ambos resultados están registrados. Un README no puede imprimir color, así que lee cada `▤` de arriba como uno verde. Una celda sin muestra no dibuja documento alguno: insinuar código donde no hay nada que abrir es peor que una celda vacía. Un `▤ —` verde significa: nuestro contrato corrió aquí y terminó limpio, y no se ha reportado ningún build en esta coordenada. `—` permanece desconocido.
...
El contrato anterior es lo que hace el código. [PRIVACY.md](../../PRIVACY.md) dice lo mismo como política, campo por campo, nombrando el archivo que impone cada límite: los documentos exactos que el modo comunidad sube, las peticiones que son descargas y no subidas, qué guarda el servidor y durante cuánto tiempo, y qué significa `local-only` cuando dice que no envía nada. Está versionada en este repositorio en lugar de servirse desde una página editable sin dejar rastro, y es la URL a la que apunta el array `privacy_policies` del paquete MCPB.
...
Código: Apache-2.0. Los samples publicados usan **MIT-0** por defecto. Evidence y datos de compatibilidad agregados: **CDLA-Permissive-2.0** (consulte [DATA_TERMS.md](../../DATA_TERMS.md)).
