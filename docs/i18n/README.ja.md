...
この行は説明用の作例ではなく、[実際のライブページ](https://codesamplex.dev/npm/axios/1.12.2/axios.post)です。linux の `PASS` は固定（pinned）コンテナ内で実行された契約であり、windows のセルは、証拠がより弱いこと（`○` の観測に `?` の不確実性マーカーが付く — 契約の実行ではなくビルドの観測であること）と、実測された失敗（`!`）を正直に示しています。不明なものは `—` のままです。パッケージのエコシステムやドキュメントから推定されるものは何もありません。
