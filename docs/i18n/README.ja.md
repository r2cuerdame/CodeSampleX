...
**セルは比率と印を持ち、判定は下しません。** 百分率とその隣の数字は観測です — 記録された 1,209 件の観測のうち 82% が通過しました。観測はビルドが到達した段階 1 つ（コンパイル・型チェック・テスト）なので、1 回のビルドが複数件を残します。ビルド数でもマシン数でも人数でもありません。`◆` の印が言うのは 1 つだけです — このネットワークが**この環境で**自らのコントラクトを実行し、問題なく終わったということ。そのリリースと API にコードが**あるかどうか**は別の印（文書アイコン）です。サンプルは OS フィルタを切り替えても消えません。意図的にチェックではありません。チェックは承認の印であり、このネットワークは等級を付けないからです。私たちの実行が失敗した場合は代わりに `✕` が付きます。`◆ —` は「私たちのコントラクトがここで走って問題なく終わり、この座標で報告されたビルドはまだない」です。`—` は不明のままです。
