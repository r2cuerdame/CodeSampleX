...
このグリッドは図解ではなく[実際のライブページ](https://codesamplex.dev/golang/github.com%2Fjackc%2Fpgx%2Fv5)です。

**セルは比率と印を持ち、判定は下しません。** 百分率とその隣の数字は実際のマシンが行ったことで、チェックは私たちのサンプルがそこで動くという意味です。私たちの実行は固定したコンテナの繰り返し、報告された千回は千通りの状況なので、両者を足しません。`✓ —` は「動くコードはあるが、使っているのをまだ誰も見ていない」です。`—` は不明のままです。
