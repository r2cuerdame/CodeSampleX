...
## ネットワークを読むのにインストールは不要

CodeSampleX はエージェント・ブラウザ・スクリプト向けの公開 REST サーフェスを公開しています。サイトが描くすべての答えは、ただの HTTPS リクエスト 1 つで手に入ります — キーも、アカウントも、クライアントライブラリも不要で、どのオリジンからでも読めます。CLI をインストールするのは、ローカル実行の記録、ビルド失敗時の自動フック、公開、ワーカー機能が必要なときだけです。

リクエスト 1 つで本番の JSON が返ります — そのままコピーしてください:

```bash
curl 'https://codesamplex.dev/v2/search?package=pkg:npm/axios&symbol=axios.post&os=linux&runtime=node&runtimeVersion=22'
```

応答には `grade` が含まれ、各結果はどの環境次元が一致したか（`exact[]`）、一致しなかったか（`different[]`）を名指しします。ミスは `grade: "NO_SAFE_MATCH"` と綴られます — 答えられなかったのではなく、それ自体が本当の答えです。汎用の Web エージェントは [`https://codesamplex.dev/skill.md`](https://codesamplex.dev/skill.md) から契約全体を読み、人は [docs/rest.md](../rest.md) で同じクイックスタートを読めます。

3 つの入口の比較です。すべてのマークはテストがルーターとコマンド一覧に照合し、[Features](https://codesamplex.dev/features) ページも同じ表を描きます:

<!-- BEGIN:CSX-SURFACE-MATRIX -->
| 機能 | Web REST（インストール不要） | MCP | CLI インストール済み |
|---|:--|:--|:--|
| 環境に対して格付けした検証済みサンプルの検索 | ✅ | ✅ | ✅ |
| パッケージ・バージョン・シンボル・環境による互換性の照会 | ✅ | ✅ | ⚠️ csx search は同期済み shard に対して格付けする。explain コマンドはない |
| サンプル 1 件のマニフェスト・レシート・ファイルの読み取り | ✅ | ✅ | ⚠️ csx search --json 経由のみ。専用のサンプル読み取りコマンドはない |
| Findings コレクションの読み取り | ✅ | ❌ Web と REST のみ | ❌ Web と REST のみ |
| gaps・wanted・公開 stats の読み取り | ✅ | ❌ | ⚠️ csx stats はローカルのカウンターだけを表示する |
| ブラウザ・クラウドエージェント・スクリプトからの利用 | ✅ | ⚠️ エージェントの MCP ホストがローカルで csx を実行している必要がある | ❌ ローカルバイナリが必要 |
| インストール不要での利用 | ✅ | ⚠️ MCP サーバーは csx バイナリそのもの。クライアントホストに csx がインストールされている必要がある | ❌ |
| ローカルのプロジェクトと環境の自動検出 | ❌ | ⚠️ MCP ホストの背後にあるローカル csx が見える範囲だけ | ✅ |
| 実際のローカルビルド・テストの実行 | ❌ | ⚠️ run_observed_command がローカル csx を通して実行する | ✅ |
| サニタイズ済みの構造化された実行証拠の取得 | ❌ | ⚠️ MCP ホストが実行するローカル csx を通してのみ | ✅ |
| 署名なし実行フットプリントの送信 | ✅ | ❌ 代わりに相関付きの adoption 証拠を送る | ❌ 代わりに相関付きの adoption 証拠を送る |
| ビルド失敗時の自動照会フック | ❌ | ❌ | ✅ |
| バックグラウンド同期とオフラインキャッシュ | ❌ | ❌ | ✅ |
| アップロード前のプライバシープレビュー | ❌ | ❌ | ✅ |
| 検証済みサンプルの公開 | ❌ | ❌ 意図的に publish ツールを持たない | ✅ 人が CLI で確認する |
| ワーカーと matrix 検証 | ❌ | ❌ | ✅ |
| 署名付き検証レシート（ワーカーの ed25519） | ❌ | ❌ | ✅ |
<!-- END:CSX-SURFACE-MATRIX -->

**REST はネットワークを読む。CLI はネットワークに現実を観測させる。** MCP は CLI の上のアダプターです: `csx mcp` は csx バイナリそのものなので、csx のないホスト上の MCP クライアントには話す相手のサーバーがありません。
## そこで動くのか？
...
**セルは比率と印 1 つを持ち、判定は下しません。** 百分率とその隣の数字は観測です — 記録された 1,209 件の観測のうち 82% が通過しました。観測はビルドが到達した段階 1 つ（コンパイル・型チェック・テスト）なので、1 回のビルドが複数件を残します。ビルド数でもマシン数でも人数でもありません。`▤` の印は文書で、文書があるということは、その座標にサンプルがあるという 1 つのことだけを意味します。その**色**が、そのサンプルを私たちがここで走らせた結果です — 灰色はここではまだ誰も走らせていない、緑は通過、赤は失敗、半分に割れた文書は通過と失敗の両方が記録されている。README は色を印刷できないので、上の `▤` はすべて緑として読んでください。サンプルのないセルには文書を一切描きません。開くものがない座標にコードがあるかのように見せるのは、空欄より悪いからです。緑の `▤ —` は「私たちのコントラクトがここで走って問題なく終わり、この座標で報告されたビルドはまだない」です。`—` は不明のままです。
...
| `GET /v1/peers/for-sample/{sampleId}` | サーバーを経由せずに取得するためのサンプル保持ピア一覧 |
| `GET /v1/wanted` | 需要キュー: 求められたのに答えられなかったもの |
| `GET /v1/adapters` | エコシステム別のケイパビリティマトリクス |
| `GET /version` | 応答したサーバーのビルドおよび環境情報 |
...
`csx init` は Claude Code、Codex、Gemini CLI、Antigravity (agy)、OpenCode を自動設定します。その他の stdio MCP クライアント（Cursor、Windsurf、Cline、Zed、VS Code）は、`csx mcp-config` が出力する設定（Codex 用は `--toml`）で動きます — この出力にはバイナリの絶対パスが含まれており、エディタから起動されるクライアントにはそれが必要です。サーバー本体は `csx mcp` です。ツールは 10 個: `search_known_solution`、`get_sample`、`explain_compatibility`、`run_observed_command`、`report_sample_adoption`、`report_anomaly`、`report_csx_issue`、`propose_public_sample`、`list_local_hits`、`get_local_stats` — そして公開（publish）ツールは意図的にありません。 `report_csx_issue` は同じ発想をパッケージではなく**私たち自身**に向けたものです。あなたが実際に見ていた失敗を押しのけた答え、質問が一度も触れていないエコシステムからの推薦、モデルを誤った行動に導いたツール契約 — そうしたものを対象にします。opt-in で、意図的に静かです。失敗のたびに呼べと指示するものはなく、チケットも作られず、報告ゼロの週は正常な週です。百人のエージェントが遭遇した欠陥は「1 行」であり、その発生回数が増えるだけです。その行が一度バグに紐づけられれば、以後の報告はその紐づけを返します。2 つのチャネルは ingest・redaction・dedupe を共有し、その先は何も共有しません。この製品の欠陥が互換性エビデンスになることは決してありません。
...
コード: Apache-2.0。公開サンプルのデフォルトは **MIT-0** です。Evidence および集計された互換性データ: **CDLA-Permissive-2.0**（[DATA_TERMS.md](../../DATA_TERMS.md) を参照）。
