...
这个网格不是示意图，而是[真实的线上页面](https://codesamplex.dev/golang/github.com%2Fjackc%2Fpgx%2Fv5)。

**单元格只带比率和标记，从不给出裁决。** 百分比和旁边的数字是真实机器的结果，勾号表示我们的样本在那里能跑。我们的运行是固定容器的重复，而上千次报告是上千种不同情境，因此不相加。`✓ —` 表示：可用代码存在，但还没有人被看到在用。`—` 保持未知。
