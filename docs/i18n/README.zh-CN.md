...
那一行不是示意图，而是[真实的线上页面](https://codesamplex.dev/npm/axios/1.12.2/axios.post)。

**单元格陈述比率并标明依据，从不给出裁决。** `■ verified` 表示我们亲自在固定的容器中执行了契约；`○ observed` 表示真实机器记录了运行并作了报告。数字就是测量本身 — 运行中的通过数 — 因此孤零零的 `1/1` 会直接说明证据有多薄，而不再躲在一个与一百次一致运行看起来完全相同的标记后面。`PASS` 已经取消：PASS 会被读成 *这里能用* 这样的一般性主张，而实际测量到的是 *运行四次，通过四次*。

依据永远不会模糊，因为那才是要紧的区分。观测数远远压过验证数，若两者都只是裸比率，匿名单元格看起来会比已证明的更权威。字形对我们亲自运行的是实心，对收到的报告是空心，不依赖颜色也能分辨；颜色只承载比率的结果 — 失败是罕见而信息量大的事件，必须抓住视线。`!` 标记测得的异常，`?` 标记薄弱或陈旧的证据，`—` 保持未知。不会从包所属的生态系统或其文档推断任何内容。
