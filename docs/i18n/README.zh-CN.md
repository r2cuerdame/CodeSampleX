...
**单元格只带比率和一个标记，从不给出裁决。** 百分比和旁边的数字是观测：已记录的 1,209 条观测中有 82% 通过。一条观测是一次构建到达的一个阶段（编译、类型检查、测试），因此一次构建会留下多条；它既不是构建数，也不是机器数或人数。`▤` 标记是一份文档，而文档只说明一件事：这个坐标上有样例。它的**颜色**才是我们在这里跑它的结果——灰色表示这里还没人跑过，绿色表示通过，红色表示失败，从中间分开的文档表示通过和失败都有记录。README 印不出颜色，所以上面的 `▤` 都请当作绿色来读。没有样例的单元格不画文档：在没有东西可打开的坐标上装作有代码，比留空更糟。绿色的 `▤ —` 表示：我们的契约在这里跑过并干净地结束，而这个坐标上还没有任何构建被报告。`—` 保持未知。
...
代码：Apache-2.0。发布的样例默认采用 **MIT-0**。Evidence 及聚合兼容性数据：**CDLA-Permissive-2.0**（参见 [DATA_TERMS.md](../../DATA_TERMS.md)）。
