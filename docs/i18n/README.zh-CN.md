...
`csx init` 会自动配置 Claude Code、Codex、Gemini CLI 和 OpenCode。其他任何 stdio MCP 客户端（Cursor、Windsurf、Cline、Zed、VS Code）只需使用 `csx mcp-config` 打印的配置即可接入（Codex 用 `--toml`）——它输出的是二进制文件的绝对路径，而这正是由编辑器启动的客户端所需要的。服务器本身就是 `csx mcp`。共九个工具：`search_known_solution`、`get_sample`、`explain_compatibility`、`run_observed_command`、`report_sample_adoption`、`report_anomaly`、`propose_public_sample`、`list_local_hits`、`get_local_stats`——并且有意不提供发布工具。
