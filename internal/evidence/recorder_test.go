	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			coord := domain.CLIExperienceCoordinate{
				...
				Environment: domain.EnvironmentFingerprint{SchemaVersion: 1, Ecosystem: domain.CommandEcosystem(tc.argv), OS: runtime.GOOS, Arch: runtime.GOARCH},
			}
			summary, err := db.QueryCLIExperience(context.Background(), coord)
			if err != nil {
				t.Fatalf("QueryCLIExperience: %v", err)
...
func TestRecordCommandOutputStoresOnlyStructuredSecretSafeCLIEvidence(t *testing.T) {
	...
	env := domain.EnvironmentFingerprint{SchemaVersion: 1, Ecosystem: "generic", OS: "windows", Arch: "x64", Runtime: "go", RuntimeVersion: "1.26"}
