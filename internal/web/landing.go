...
func buildHeroEcos(lang string) []heroEco {
	...
	for _, e := range landingEcosystems {
		ecos = append(ecos, heroEco{Name: e, Href: compatibilityHref(RecordFilter{Ecosystem: e}, 1, lang)})
	}
	return ecos
}
...
// statsPage points at /compatibility directly. It used to redirect to
// /explore, itself a 301: measured on the live site, /stats was
// a two-hop chain (301 → 301), and a chain is a link that only partly
// arrives.
func (s *site) statsPage(w http.ResponseWriter, r *http.Request) {
	s.redirectTo(w, r, "/compatibility")
}
