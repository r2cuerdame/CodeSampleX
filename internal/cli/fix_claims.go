...
func fixClaimsUsage() {
	...
	fmt.Fprintln(w, "       csx fix-claims probe --repro DIR --version V [--id ID] [--os linux] [--runtime R --runtime-version RV] --server URL --token TOKEN")
	fmt.Fprintln(w, "         one probe by hand; without --id everything executes and nothing is filed")
