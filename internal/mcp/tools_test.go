...
func TestRunObservedCommandRoundTrip(t *testing.T) {
	deps := emptyDeps()
	deps.RunObserved = func(_ context.Context, argv []string, cwd string) (int, string, string, []string, commandOutput, error) {
		...
		return 1, "PROJECT_COMPILE", "FAIL", []string{"errorCode: TS2345", "error TS2345: <path> is not assignable"}, commandOutput{}, nil
	}
	c := startServer(t, deps)
