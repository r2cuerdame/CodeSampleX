...
// then discarded. On a 2-vCPU burstable host that work is also what exhausts
// the CPU credits, and the throttle that follows is charged to every page on
// the site, so the waste is not confined to the panel that asked for it.
//
...
func (h *handler) coverage(ctx context.Context, now time.Time) ([]serverstore.FarmAxisCoverage, time.Time) {
	...
	if err != nil {
		...
		h.farmCoverage.retryAt = now.Add(farmCoverageBackoff)
		return h.farmCoverage.value, h.farmCoverage.at
	}
	h.farmCoverage.value = value
	h.farmCoverage.at = now
