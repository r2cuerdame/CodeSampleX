...
| `doctor` | diagnose the local install; `--fix` repairs safe CSX-owned state and verifies again |
...
Host IDEs, coding agents, and developer platforms can expose CodeSampleX as one
built-in or recommended option without exposing connector details. The
[partner integration contract](docs/partner-integration.md) and its
[machine-readable profile](integrations/partner-profile.json) define the
managed-package lifecycle, consent boundary, capability policy, and evidence
